...
from peft import LoraConfig, prepare_model_for_kbit_training, set_peft_model_state_dict, AdaLoraConfig
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--target_r", type=int, default=None, help="Target rank for AdaLoRA pruning (default: rank//4)")
    parser.add_argument("--init_r", type=int, default=None, help="Initial rank for AdaLoRA (default: same as rank)")
    parser.add_argument("--tinit", type=int, default=None, help="Step to start pruning (default: total_steps//4)")
    parser.add_argument("--tfinal", type=int, default=None, help="Step to stop pruning (default: 3*total_steps//4)")
    parser.add_argument("--deltaT", type=int, default=10, help="Pruning frequency in steps")
    parser.add_argument("--beta1_adalora", type=float, default=0.85, help="EMA parameter 1 for AdaLoRA")
    parser.add_argument("--beta2_adalora", type=float, default=0.85, help="EMA parameter 2 for AdaLoRA")  
    parser.add_argument("--orth_reg_weight", type=float, default=0.5, help="Orthogonality regularization weight")
    parser.add_argument("--use_adalora", action="store_true", help="Use AdaLoRA instead of regular LoRA")
...
def main(args):
    ...
    # Create LoRA or AdaLoRA config based on flag
    if args.use_adalora:
        # Set default values for AdaLoRA parameters
        target_r = args.target_r if args.target_r is not None else args.rank // 4
        init_r = args.init_r if args.init_r is not None else args.rank
        tinit = args.tinit if args.tinit is not None else total_steps // 4
        tfinal = args.tfinal if args.tfinal is not None else 3 * total_steps // 4
        
        transformer_lora_config = AdaLoraConfig(
            r=args.rank,                    # Maximum rank
            target_modules=target_modules,  # Which layers to adapt
            lora_alpha=args.lora_alpha,    # LoRA scaling parameter
            lora_dropout=args.lora_dropout, # Dropout rate
            init_lora_weights="gaussian",   # Weight initialization
            target_r=target_r,             # Target rank after pruning
            init_r=init_r,                 # Initial rank
            tinit=tinit,                   # Step to start pruning
            tfinal=tfinal,                 # Step to finish pruning  
            deltaT=args.deltaT,            # Pruning frequency
            beta1=args.beta1_adalora,      # EMA parameter 1
            beta2=args.beta2_adalora,      # EMA parameter 2
            orth_reg_weight=args.orth_reg_weight, # Orthogonality weight
            total_step=total_steps,        # Total training steps (required!)
        )
        print(f"Using AdaLoRA: rank {args.rank} -> {target_r}, pruning from step {tinit} to {tfinal}")
    else:
        # Use regular LoRA
        ...
        print(f"Using regular LoRA with rank {args.rank}")

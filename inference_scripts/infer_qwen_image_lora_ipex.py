...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the Qwen-Image model checkpoint (e.g., 'Qwen/Qwen-Image').",
    )
    parser.add_argument(
        "--tokenizer_path",
        type=str,
        default=None,
        help="Path to the updated tokenizer checkpoint.",
    )
    parser.add_argument(
        "--text_encoder_path",
        type=str,
        default=None,
        help="Path to the text encoder checkpoint.",
    )
    parser.add_argument(
        "--token_abstraction_json_path",
        type=str,
        default=None,
        help="Path to the token abstraction dictionary JSON file.",
    )
    parser.add_argument(
        "--transformer_lora_path",
        type=str,
        default=None,
        help="Path to the transformer LoRA checkpoint (.safetensors).",
    )
    parser.add_argument(
        "--num_inference_steps",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Data type for model weights (bf16 recommended for CPU with IPEX).",
    )
    parser.add_argument(
        "--true_cfg_scale",
        type=float,
        default=4.0,
        help="Classifier-free guidance scale for text conditioning.",
    )
    parser.add_argument(
        "--instruction",
        type=str,
        default=None,
        help="Text prompt for image generation (single prompt mode).",
    )
    parser.add_argument(
        "--negative_prompt",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="output.png",
        help="Base path to save generated images (will append indices and timestamp).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="16:9",
        help="Aspect ratio for generated images (e.g., '16:9', '1:1').",
    )
...
def convert_lora_weights_before_load(args: argparse.Namespace, state_dict: dict, new_path: str) -> dict:
    if args.token_abstraction_json_path:
        state_dict.pop("emb_params", None)  # Remove if present
...
def main(args: argparse.Namespace, prompts: List[str]) -> None:
    ...
    width, height = aspect_ratios.get(args.aspect_ratio, (1664, 928))  # Default to 16:9
